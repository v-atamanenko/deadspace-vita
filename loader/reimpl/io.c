...
char* fix_path(const char * orig_path) {
    char* path_temp = (char*)malloc(1024*sizeof(char));
...
int read_soloader(int __fd, void *__buf, size_t __nbyte) {
    //debugPrintf("[io] read(%i, %x, %i)\n", __fd, (int)__buf, __nbyte);
...
int fseeko_soloader(FILE * a, off_t b, int c) {
    debugPrintf("[io] fseeko(%i)\n", (int32_t)a);
    return fseeko(a,b,c);
}

off_t ftello_soloader(FILE * a) {
    debugPrintf("[io] ftello(%i)\n", (int32_t)a);
    return ftello(a);
}
